...
for time in range(60):
    ...
    bins = {}
    next_states = list()
    for each_state in range(len(current_state)):
        cur_state = current_state[each_state]
        level_set_t1.append(cur_state)
        level_set_actions.append(actions)
        for each_action in range(actions.shape[0]):
            action = actions[each_action]
            next_state = dynamics_random_walk(cur_state, action)
            bins[tuple(next_state)] = 0
            if not any(np.array_equal(next_state, st) for st in next_states):
                next_states.append(next_state)
    level_sets.append(level_set_t1)
    level_sets_actions.append(level_set_actions)
    bins_level_set.append(bins)
    current_state = copy.deepcopy(next_states)

print(len(level_sets))
...
for each_level_set_bin in range(len(bins_level_set)):
    ...
    if each_level_set_bin > 50:
        print("\n")
        print(bins_level_set[each_level_set_bin])
...
plt.savefig("box_plots_distributions_NN_extrapolation_nf_prob.png")
plt.show()