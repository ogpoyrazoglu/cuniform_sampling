...
for each in range(len(level_sets)):
    ...
    for each_state in range(len(current_probabilities)):
        ...
        if each < 5:
            print(each, current_state)
            print("NF Prediction", current_probability_distribution)
        num_samples = 5000
        current_probability_distribution = nn.Softmax(dim = 1)(model(current_state))
        if each < 5:
            print("NN Prediction", current_probability_distribution)
            print("\n")
...
plt.savefig("box_plots_distributions_NN_nf_prob.png")
plt.show()